...
function getFieldValuesForSubmission($db, $submissionId) {
    $sql = "SELECT ff.field_key, ff.field_label, ffv.field_value
...
try {
    ...
    // 获取所有提交数据（不含 data 列，新表结构）
    $stmt = $db->query("SELECT id, submission_id, form_id, form_name, ip, user_agent, submitted_at 
    ...
    foreach ($submissions as &$sub) {
        $sub['data'] = getFieldValuesForSubmission($db, $sub['id']);
    }
    
    // 生成 JSON 供 JavaScript 使用
    $submissionsJson = json_encode($submissions, JSON_HEX_TAG | JSON_HEX_APOS | JSON_HEX_QUOT | JSON_HEX_AMP);
    
} catch (Throwable $e) {
    $total = 0;
    $submissionsJson = '[]';
    error_log('forms.php database error: ' . $e->getMessage());
}
