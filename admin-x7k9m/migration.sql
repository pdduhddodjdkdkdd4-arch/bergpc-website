...
CREATE TABLE IF NOT EXISTS `form_field_values` (
  ...
  `field_id` BIGINT UNSIGNED DEFAULT NULL COMMENT '关联 form_fields.id(可选)',
  `field_label` VARCHAR(255) DEFAULT NULL COMMENT '字段标签(显示名称)',
  `field_value` TEXT DEFAULT NULL COMMENT '字段值',
  `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
  KEY `idx_submission` (`submission_id`),
  KEY `idx_field` (`field_id`),
  KEY `idx_label` (`field_label`),
...
INSERT IGNORE INTO `form_fields` (`form_id`, `field_key`, `field_label`, `field_type`, `is_searchable`, `display_order`) VALUES
